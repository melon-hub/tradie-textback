    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Invalid login credentials')) {
          return { 
            success: false, 
            error: 'Dev users not found. Please run the setup script in Supabase SQL Editor: scripts/setup-dev-users-simple.sql' 
          };
        }
        ...
        if (error.message.includes('Email logins are disabled')) {
          return { 
            success: false, 
            error: 'Password authentication is disabled. Enable it in Supabase Dashboard > Authentication > Providers > Email' 
          };
        }
