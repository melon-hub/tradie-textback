-- Safe version of populate test data script
-- First, let's check what values are allowed for various fields

-- Check current job statuses
SELECT DISTINCT status FROM jobs;

-- Check current job urgencies  
SELECT DISTINCT urgency FROM jobs;

-- Check job_photos structure and any existing upload_status values
SELECT DISTINCT upload_status FROM job_photos;

-- Now populate the data with known safe values
...
-- Skip job_photos for now until we know the correct upload_status values
-- We can add them manually later or check the constraint first
...
-- Verify the data
SELECT 'Test data population complete (without job_photos)!' as message;
